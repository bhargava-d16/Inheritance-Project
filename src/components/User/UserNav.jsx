  ...
  const handleScheduledMeetingsClick = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/scheduled-meets/${sharedUsername}`
      );
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg w-1/3 p-6">
            ...
            {scheduledMeets && scheduledMeets.length > 0 ? (
              ...
              // Ensuring the "No meetings scheduled" message is always shown
              // <p className="text-gray-700 text-center py-4">No scheduled meetings found.</p>
              // toast.error("No scheduled meetings found.")
              <></>
            )}
            <button
              onClick={() => setShowScheduledMeetsPopup(false)}
              className="mt-4 px-4 py-2 bg-[#133E87] text-white rounded-md hover:bg-[#608BC1]"
            >
              Close
            </button>
          </div>
