  ...
  try {
    ...
    if (!u) {
      return res.status(403).json({
        msg: "Invalid Credentials Or User Does't Exist",
        success: false,
      });
    }
    ...
    if (x) {
      const jwtToken = jwt.sign(
        { username: u.username, _id: u._id },
        process.env.JWT_SECRET,
        { expiresIn: "24h" }
      );
      res
        .status(201)
        .json({ msg: "Login Success!", success: true, jwtToken, username });
      // Creating jwt token
...
const getUserProfile = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message, hello: "gand mara madarchod" });
  }
};
